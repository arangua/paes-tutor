...
describe('POST /api/notes/versions/compress', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('debe retornar 401 si el usuario no está autenticado', async () => {
    ...
    const request = createTestRequest({
      baseUrl: 'http://localhost/api/notes/versions/compress',
      method: 'POST',
      body: {
  ...
  it('debe comprimir versiones antiguas correctamente', async () => {
    await setupAuthenticatedUser({ studentId: TEST_IDS.STUDENT })
    ...
    vi.mocked(prisma.studyNoteVersion.findMany).mockResolvedValue([
      {
        id: TEST_IDS.VERSION,
        content: 'Contenido largo que se puede comprimir '.repeat(100),
    ...
    expect(data.compressedCount).toBeGreaterThan(0)
    expect(data.spaceSaved).toBeDefined()
  })

  it('debe retornar mensaje si no hay versiones para comprimir', async () => {
    await setupAuthenticatedUser({ studentId: TEST_IDS.STUDENT })
    ...
    expect(data.compressedCount).toBe(0)
    expect(data.message).toContain('No hay versiones para comprimir')
  })
})
