  ...
  return withRateLimit(request, async () => {
    try {
      ...
      if (!Array.isArray(allStudents)) {
        logger.warn({ allStudents }, 'shared-materials: allStudents no es un array válido')
        return NextResponse.json(
      ...
      const otherStudent = (() => {
        ...
        if (!safeCurrentStudentId) {
          logger.warn({ dbUser }, 'shared-materials: dbUser.student.id no es válido')
          return null
        }
        try {
          ...
          logger.warn({ error, allStudents }, 'shared-materials: Error al ejecutar find() en allStudents')
          return null
        }
      })()
      if (!otherStudent) {
